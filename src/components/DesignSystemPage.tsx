        ...
        <div className="ds-work" id="ds-content" tabIndex={-1}>
          ...
          <section id="components" className="ds-section">
            ...
            <div className="ds-block">
              ...
              <p className="ds-caption">
                The booking conversation has a fixed circular 44px close control with <code>corner-shape: round</code> at the top-right of the white backdrop, using the same ring and control shadow as its calendar Back button; clicking the backdrop or pressing Escape also dismisses it.
                ...
                at the viewport limit. Hints, email editing, and receipts use <code>--muted</code> text. Incoming bubbles use flat <code>--mosaic-card-surface</code> gray with <code>--ink</code> text, no border or shadow, and compact 8px by 12px padding;
                right-aligned replies use the existing Messages blue <code>#0071e3</code> with white text.
                The Messages-inspired header stays fixed 20px from the viewport top (respecting safe areas), centered over the conversation column independently of message growth. The conversation reserves top space so scrolling messages cannot overlap it. On opening, the visible homepage portrait’s bounds are captured and the chat image translates and scales from those bounds over <code>--duration-slow</code> with <code>--ease-smooth</code>; reduced motion and an offscreen homepage portrait skip the flight. The header centers a 64px circular portrait (48px on short screens) with <code>corner-shape: round</code> and a subtle <code>--shadow-ring</code>. The portrait sits in front of the white <code>--radius-full</code> name pill at their 8px overlap, using local <code>z-index: 1</code> inside an isolated identity stack. The pill reads “Rafael” with round corners, a soft <code>--shadow-control-hover</code> without a border ring, and semibold <code>--text-sm</code> lettering. Its 14px chevron uses <code>--muted-soft</code> at 60% opacity.
                The name pill is 36px tall with 6px vertical and 12px horizontal padding, plus an invisible 4px extension above and below for a 44px contact-info target. Its nested contact sheet slides from the right over <code>--duration-slow</code> and <code>--ease-smooth</code>, exits over <code>--duration-quick</code>, and stays within 12px safe-area gutters. At 900px and above the panel occupies the right half and the conversation moves into the left half; on phones it uses a maximum width of 360px. Grouped details stay centered in a 400px column with white <code>--radius-md</code> rows, inset separators, <code>--text-xs</code> muted labels, and <code>--text-md</code> values. The pale sheet mixes 45% <code>--mosaic-card-surface</code> with <code>--canvas</code> and uses the existing Messages blue for links. Both nested layers use <code>--z-dialog</code> in portal order, with a 6% black veil. Escape closes contact info first and restores the name trigger; reduced motion removes the slide.
                Bubbles use <code>--text-sm</code> with round <code>--radius-md</code> corners and 8px gaps; outgoing replies retain a <code>--radius-sm</code> bottom-right corner. They borrow the side chat’s 8px rise, 0.98 scale, and 2px blur over <code>--duration-slow</code> with <code>--ease-smooth</code>: the opening bubbles begin at 200ms, 360ms, and 520ms after opening, followed by the composer at 680ms. Follow-up bubbles wait 80ms. Closing resets the entrance so it replays on reopening. New replies enter individually; delivery updates do not replay them. Fields stay on <code>--text-md</code> (16px, including on phones). The compact circular booking button sits beside the message composer with an 8px gap, a 44px target, and a 20px calendar icon; its accessible label and tooltip identify the action. Hints use <code>--text-xs</code>.
                The iOS-style white composer uses truly round <code>--radius-md</code> corners, a fine <code>--shadow-ring</code>, and subtle <code>--shadow-control</code> elevation, lifting to <code>--shadow-control-hover</code> on focus. Its text has 16px horizontal padding, no gap before the trailing icon, and a Messages-blue caret. Send and its inset surface also use <code>corner-shape: round</code>. The composer has no outer padding, matching the adjacent calendar button’s 44px resting height. The message field grows from one line to <code>min(188px, 30dvh)</code>, then scrolls. The empty message field shows a decorative 24px microphone in a 44px slot, using <code>--muted-soft</code> at 60% opacity; typing replaces it with the blue send control. It does not request microphone access. The 44px send target holds a 36px inset surface, gray while disabled and blue when ready. Email comes first, with Change email directly beneath the submitted blue address. Optional messages follow with receipts and retries; the message composer has no inbox caption.
                Book a time expands the same floating surface to 1104px over <code>--duration-slow</code>
                and centers a light Cal.com calendar with the email prefilled. The fixed top-left Back control restores the conversation and draft.
