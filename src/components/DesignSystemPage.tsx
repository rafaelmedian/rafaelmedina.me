...
const DURATIONS_ENTRIES = [
  ...
  { value: "800ms / 100ms", use: "The personal-photo fan dealing itself in the first time it scrolls into view: each print rises a tenth of its height over 800ms on a plain ease-out, staggered 100ms per step out from the middle print. Taken whole from the arc gallery hero it follows, so it is slower than the house entrances on purpose. --photo-deal-duration and --photo-deal-stagger on the stack." },
      ...
      <div className="ds-app">
        ...
        <div className="ds-work" id="ds-content" tabIndex={-1}>
          ...
          <section id="components" className="ds-section">
            ...
            <div className="ds-block" data-ds-terms={terms("personal photos tile fan arc stack pebble glass globe sphere webgl modal shadow radius grid masonry toggle layout hold caption")}>
              <p className="ds-subhead">Personal photos</p>
              <p>The preview is a tile in the work grid, the band under the quote and Security that closes the portraits group, on the writings folder’s chrome: a 1px 8% black border, --radius-lg corners, the --mosaic-card-surface fill, and a --text-sm “Personal life” label under the fan. The square-cropped prints number four below 700px and five at 700px and above, each advancing half its own width so the fan overlaps hard. They are always the first photos: no visit reshuffles the stack, so the sheet always opens at its first row and always folds back into the same prints. The count also updates when the viewport changes. Prints stay at most 35% of the fan’s width and shrink to fit the five-print row; a shorter row centres rather than hanging off the left. The fan is capped at 27rem (20rem below 700px) and centred in the tile, with a 2.25rem gap below it (1.5rem on phones) so the outer prints’ corners clear the label. They are dealt along an arc rather than jumbled: the lean runs straight from -10deg at the left end of the row to +10deg at the right, each print drops by the square of its distance from the middle up to 10% of its own height, and the middle print sits at the front of the pile with each one behind it stepping back — so the fan opens outwards instead of shingling left to right. The tile appears with the work grid after the avatar intro, and the first time it scrolls into view the hand deals itself in, after the intro of 21st.dev’s arc gallery hero: each print fades in as it rises a tenth of its own height, 800ms on a plain ease-out, from the middle out — the front print first, then 100ms later for each step towards the ends (the two middle prints of a four-print phone row share the first beat). A fan already on screen when the page loads, or above it, has been seen and stays put; it is dealt once per visit; a click that opens the sheet mid-deal snaps the hand to rest before the flights measure it; and reduced motion never holds it back. A pointer moving over the tile opens the whole hand at once: every print swings out to its fanned angle — the same arc widened to -16deg and +16deg with the drop deepened to 16% — over --duration-slow (360ms) on --ease-smooth. Only travel opens it: the hand never opens on :hover or on the focus the sheet hands back, so it is at rest at both ends of a flight and stays at rest after a close until the pointer moves again, and keyboard focus shows the house ring alone. Nothing changes hands under the pointer and the region each print answers to never moves. Each print keeps the original --shadow-overlay card shadow plus a 6% hairline, and its 1:1 image crop sits slightly above center to keep faces in view. Clicking or tapping a print opens the globe holding that photo at its centre; a click beside the prints, or Enter, opens it as it lies. A fine pointer can pull a print first: pressed, it follows the pointer with a rubber band's resistance (a pull of p pixels moves it p x 48 / (48 + p) — 100px pulls 32, 200px 39) and turns up to 16deg with the pull; let go, its photo takes off for the globe from where the pointer left it while the print springs back into the hand over --duration-slow on --ease-smooth. Touch and reduced motion keep the plain tap. Opening turns every photo into a tile on a globe — the photo globe, a Fibonacci-sphere layout after 21st.dev’s img-sphere, with the photos as the glass pebbles of the material study rather than in circles. Every photo appears at least twice (copies are decoration: no id, aria-hidden, out of the Tab order), because a few dozen photos on a scatter read as a cloud and the globe only reads as one when it is covered; the count of copies is Math.max(2, round(36 / photos)). The globe is min(94vw, 88vh, 56rem) wide (min(118vw, 72vh) on phones, overhanging both edges equally) with the sphere at 0.4 of that (0.36 on phones); each tile at the front is a share of the globe’s width that falls with the square root of the tile count (0.2 at 36 tiles; 1.3x on phones). A camera four radii out draws the front a third larger than the middle, and each tile shrinks further towards the rim (0.28 + 0.72 x depth^2.2, taken down from its own size so nothing is upscaled). The far side is hidden, not seen through: past the rim a tile fades on the square of its remaining distance and shrinks to 60%, then is display: none, so no flight aims at it; only the face answers the pointer. Every tile is drawn on one WebGL 2 canvas as the study’s glass pebble — a domed face, a semicircular shoulder the photograph wraps over, a narrow fresnel rim and a soft off-axis reflection, no frame — leaning with the globe’s surface by two fifths of its angle so the fixed light travels along each shoulder as the globe turns; the slides underneath keep the pointer, the focus ring and the boxes the flights aim at, and draw a CSS pebble (squircle corners at 21% via corner-shape, inset glint and shade) wherever WebGL 2 is missing or until each photo is on the GPU. The far side darkens with depth ((1 - depth)^1.6 x 0.72) rather than going transparent. The page behind is frosted: the backdrop is 60% #121212 with an 18px backdrop-filter blur, because the work grid’s phones and dashboards read as more photos when left sharp. Motion: the globe spins on its own at 0.16 rad/s (a turn in about 40s), easing in and out; it waits out the open flight (200ms), stops under a pointer resting on a photo, while a photo is focused from the keyboard or held, and for 4s after a photo is brought to the front. A drag anywhere on the stage turns it about the screen’s own axes (0.006 rad/px) with momentum decaying at 0.94 per 60th of a second; the wheel and a trackpad turn it too; the arrow keys step it 30deg over --sphere-focus-duration (--duration-slow, 360ms, ease-out cubic), which is also the settle time of the hold's spring. A press that travels more than 6px is a drag, never a click. Hovering a photo on the face grows it 8% under a pointer cursor and brings it forward of its neighbours — the first 6% of the way to its held size, so a click carries on from it. Clicking a photo holds it on one critically damped spring, settling to 95% in --sphere-focus-duration (about 13/s): the turn to the centre, the growth to 2.4x its front size, the rest of the globe receding 22%, the restacking and its pebble squaring up all ride the same value, so they start from rest together and land together, and the spring keeps its speed through a change of mind, so a photo let go or handed over mid-way turns round rather than stopping first; the held photo leans towards the pointer by up to 0.2rad on the same spring. While a photo is held, the next click anywhere — the photo, a neighbour, or the margin — lets it go; a drag lets it go too. With nothing held, a click on the margin closes the globe. Tab moves through the originals in photo order and turns each to the front; Escape lets a held photo go, and closes otherwise. One caption names the held photo, just under it (above it when the screen ends below), in --font-ui at --text-md (--text-sm on phones) weight 500 in --canvas: placed from the photo’s drawn box every frame and faded in over the last two fifths of the photo’s zoom, so it rises 8px into place as the hold lands and is gone the moment a release starts the photo back; nothing is named while nothing is held. Each tile’s own figcaption stays for assistive tech. The open and close flights are the fan’s: every print flies to its slot on the face of the globe and back, and the other photos on screen borrow the nearest print, over --photo-open-duration / --photo-close-duration (200ms, --photo-motion-ease); the globe holds still from the moment a close begins so the flights are measured against slots that do not move, and a flight lands on a slide drawn at whatever scale the globe gives it. Reduced motion removes the spin, the momentum and the eased turns (a drag still turns it, and the hold and the arrow steps take effect at once), and the flights.</p>
              <p>The homepage fan also tilts toward a fine pointer, up to 10deg on each axis with 1000px perspective: as one hand while the pointer is over the label or the margin, and over a print that print alone — the hand lies back flat and the print under the pointer turns on the individual rotate property, about an axis square to the pointer's offset from its own centre, so the lean and the drop on transform and the lift on translate are untouched. Its stationary outer wrapper supplies the pointer coordinates; the hand and the print follow over --duration-base (200ms) and return over --duration-slow (360ms), both on --ease-smooth. Each print keeps its original depth throughout, and the one print under a fine pointer slides up on its own by about a tenth of its height (--print-lift, -11% with a per-print wobble of up to 1.2%) over --duration-slow on --ease-smooth, with its hit area extended down over where it rose from so a pointer near its bottom edge does not lose it; nothing is scaled or reordered. At rest every print already sits off the even arc by its fixed wobble (up to 1.6deg and 0.65% of its height), so the hand reads as dealt rather than printed. Preview corners are 11% with circular rounding, or 21% with corner-shape: squircle; the image and glass inherit the same corners. A restrained radial reflection follows the pointer over the photo, while the thin upper glint uses the existing 55% white inset highlight. The tilt resets synchronously before opening the globe, stays flat while its photos are away, and also resets on scroll, resize, pointer cancellation, or window blur. Touch keeps native scrolling and reduced motion disables this tilt.</p>
              <p>The open sheet has two layouts, picked by a Grid | Sphere toggle centred at the top of the stage (max(1.25rem, safe-area-inset-top) down): a pill on the floating-chrome recipe — 92% white, --shadow-ring plus --shadow-overlay, --radius-full, --font-ui at --text-sm — with 3px of padding, 36px segments carrying a 16px lucide icon, and a --ink thumb that slides to the picked side over --duration-base on --ease-smooth (at once under reduced motion). The picked segment reads --canvas on the thumb and the other --muted, darkening to --ink on hover; the focus ring sits inside the segment in its own text colour, since the house ink ring vanishes on the thumb. It is a group of two aria-pressed buttons, outside the stage so the globe’s drag never swallows its click. The globe is the default; the choice is kept in localStorage, so a visitor returns to the layout they left. A switch keeps the same stage — it never replays the open flight — starts the new layout at its top, and fades it in over --duration-base on --ease-smooth. The grid is the sheet before the globe: every photo once, dealt round-robin into three columns (two below 700px) so the prints run across the first row, 64rem wide at most inside a clamp(1.25rem, 4vw, 5rem) gutter, with 5rem above for the toggle and 6rem below. Columns and rows sit 1.5rem apart (1rem on phones); the captions are read out, not shown, until a photo is held. Each photo keeps its own aspect ratio and the globe’s CSS pebble, its corner at the globe’s share of min(column width, 12rem). A photo under a fine pointer grows 8% over --duration-base, as on the globe; a click, Enter or Space holds it: it glides from where it lies to the centre of the stage over --sphere-focus-duration on --ease-smooth and grows to fill 70% of the stage on its longer side (the move and growth are measured against the stage’s client box, so a photo at an edge or below the fold comes to the middle of the screen), with its caption 0.75rem under it in the same face as the globe’s, fading in once the glide is half done and out at once. The next click on any photo, Escape, or a scroll lets it go; a click on the margin lets it go, and the next one closes; a close asked for over a held photo snaps the grid to rest before the flights measure it. The grid scrolls natively, and its margin closes when nothing is held. A scrolled grid glides back to its first row over --photo-rewind-duration (--duration-slow) before the prints fly home.</p>
              <PersonalPhotos />
            </div>
