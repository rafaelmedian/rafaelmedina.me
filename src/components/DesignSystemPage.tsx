          ...
          <section id="components" className="ds-section">
            ...
            <div className="ds-block">
              ...
              <p className="ds-caption">
                ...
                The Messages-inspired header centers a 64px circular portrait (48px on short screens) with <code>corner-shape: round</code> and a subtle <code>--shadow-ring</code>. A white <code>--radius-full</code> name pill overlaps its lower edge by 8px, with round corners, <code>--shadow-overlay</code>, and semibold <code>--text-sm</code> lettering.
                The name is a 44px contact-info trigger with a muted chevron. Its nested contact sheet slides from the right over <code>--duration-slow</code> and <code>--ease-smooth</code>, exits over <code>--duration-quick</code>, and stays within 12px safe-area gutters at a maximum width of 360px. The sheet uses <code>--mosaic-card-surface</code>, white <code>--radius-md</code> grouped rows, a 96px round portrait, <code>--text-lg</code> name, and the existing Messages blue for contact links. Both nested layers use <code>--z-dialog</code> in portal order, with a 6% black veil. Escape closes contact info first and restores the name trigger; reduced motion removes the slide.
