...
test("Notes stays aligned and its breadcrumb returns to the list", async ({ page }) => {
  ...
  const like = dialog.getByRole("button", { name: "Like this note" })
  await expect(like).toBeEnabled()
  await expect(dialog.getByRole("status", { name: "Note likes" })).toHaveText(/\d+ likes?/)
  await like.click()
  await expect(dialog.getByRole("button", { name: "Unlike this note" })).toHaveAttribute("aria-pressed", "true")
  ...
  await dialog.getByRole("button", { name: "Designing Matcha", exact: true }).click()
  await expect(dialog.getByRole("button", { name: "Unlike this note" })).toHaveAttribute("aria-pressed", "true")
  await dialog.getByRole("button", { name: "Unlike this note" }).click()
