...
test("a layout switch stays opaque and lands within a fast beat", async ({ page }) => {
  ...
  expect(Math.max(...flightDurations)).toBeLessThanOrEqual(150)
  await expect(page.locator(".personal-photos-flight")).toHaveCount(0, { timeout: 300 })
})
