...
for (const width of [1440, 390]) {
  test(`the wall returns a selected photo before Escape closes it at ${width}px`, async ({ page }) => {
    ...
    await expect(photos.nth(1)).toHaveCSS("opacity", "0.12")
...
test("wall keyboard focus contrasts with the canvas and selection resets before reopening", async ({ page }) => {
...
for (const width of [1440, 390]) {
  test(`Wall arrows shift the composition around the next centred photo at ${width}px`, async ({ page }) => {
    ...
    await expect.poll(async () => {
      ...
      return Math.hypot(box.x + box.width / 2 - width / 2, box.y + box.height / 2 - (width < 700 ? 450 : 402))
    }).toBeLessThan(2)
...
for (const width of [1440, 390]) {
  test(`Wall vertical arrows keep selecting in their direction at ${width}px`, async ({ page }) => {
    ...
    const main = wall.locator(".personal-photos-slide[data-held]")
    const plane = wall.locator(".personal-photos-masonry")
    const columnX = await plane.evaluate(el => parseFloat((el as HTMLElement).style.getPropertyValue("--wall-focus-x")))
    ...
    for (const key of ["ArrowDown", "ArrowUp"]) {
      for (let i = 0; i < 6; i++) {
        ...
        const x = await plane.evaluate(el => parseFloat((el as HTMLElement).style.getPropertyValue("--wall-focus-x")))
        expect(Math.abs(x - columnX)).toBeLessThan(1)
        previousY = y
        await expect.poll(async () => {
          const box = (await main.boundingBox())!
          return Math.abs(box.x + box.width / 2 - width / 2)
