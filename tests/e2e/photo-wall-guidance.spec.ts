...
for (const width of [1440, 390]) {
  test(`photo guidance lasts three seconds after interaction and returns only on refresh at ${width}px`, async ({ page }) => {
    ...
    await dialog.getByRole("button", { name: "Close photo wall" }).focus()
...
test("drag threshold and reduced-motion keyboard zoom keep guidance predictable", async ({ page }) => {
  ...
  await page.mouse.move(401, 400)
  await page.clock.runFor(3100)
  await expect(hint).toBeVisible()
  await page.mouse.move(440, 430, { steps: 4 })
  await page.mouse.up()
  await expect(hint).toBeVisible()
  await page.clock.runFor(3100)
  await expect(hint).toBeHidden()
